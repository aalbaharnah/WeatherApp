...
app.controller("MyWeatherController", function ($scope, $http) {
    ...
    $scope.getMeHome = function () {
        $http.get('http://api.apixu.com/v1/current.json?key=46114ce99ca14871b8963021191301&q=' + localStorage.getItem('name'))
    ...
    $scope.search = function () {
        $http.get('http://api.apixu.com/v1/current.json?key=46114ce99ca14871b8963021191301&q=' + $scope.cityName)
        .then(function(response){
            console.log(response.data);    
            $scope.cityData = response.data.location.name;
            $scope.cityTemp = response.data.current.temp_c + '°';
